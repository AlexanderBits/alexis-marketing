        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white text-[10px] font-black uppercase tracking-widest">Garimpo Inteligente & Auditoria Moz V2</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex bg-slate-950 p-1 rounded-2xl border border-slate-800 shadow-xl">
            <button 
              ...
              className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${searchMode === "mining" ? "bg-indigo-600 text-white shadow-lg" : "text-white hover:text-white"}`}
            >
              Garimpar Lista
            </button>
            <button 
              onClick={() => setSearchMode("direct")}
              className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${searchMode === "direct" ? "bg-indigo-600 text-white shadow-lg" : "text-white hover:text-white"}`}
            >
              Pesquisa Direta
            </button>
          </div>
        ...
        <Card className="bg-slate-900 border-slate-800 backdrop-blur-sm p-8 mb-12 rounded-3xl relative overflow-hidden group shadow-2xl">
          ...
          <div className="grid lg:grid-cols-2 gap-8 relative z-10">
            ...
            <div className="space-y-6">
              {searchMode === "mining" ? (
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-white uppercase tracking-widest">Palavras-Chave (Registro.br)</label>
                  <div className="relative">
                    <Filter className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white" />
                    <Input 
                  ...
                  <p className="text-[10px] text-white font-bold uppercase tracking-tighter italic">Deixe vazio para minerar domínios aleatórios da lista de liberação.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-white uppercase tracking-widest">Domínios para Auditoria Direta</label>
                  <textarea 
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-white uppercase tracking-widest">Mín. PR</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-white uppercase tracking-widest">Máx. PR</label>
                ...
                <div className="space-y-2 lg:col-span-1 col-span-2">
                  <label className="text-[10px] font-black text-white uppercase tracking-widest">Ranking Global</label>
                  <div className="relative">
                    <Trophy className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white" />
          ...
          {stats.analyzed > 0 && (
            <div className="mt-8 pt-6 border-t border-slate-800 flex items-center justify-between text-white">
                ...
                <Table>
                  <TableHeader className="bg-slate-900/80 backdrop-blur-sm">
                    <TableRow className="border-slate-800">
                      <TableHead className="text-white font-black uppercase text-[10px] tracking-[0.2em] pl-10 py-6">Domínio</TableHead>
                      <TableHead className="text-white font-black uppercase text-[10px] tracking-[0.2em]">PageRank</TableHead>
                      <TableHead className="text-white font-black uppercase text-[10px] tracking-[0.2em]">Rank Global</TableHead>
                      <TableHead className="text-white font-black uppercase text-[10px] tracking-[0.2em] text-right pr-12">Ações</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {domainsList.map((item) => (
                      <TableRow key={item.id} className="border-slate-800 group hover:bg-slate-800/20 transition-colors">
                        <TableCell className="pl-10 py-7">
                          <div className="flex items-center gap-4">
                            <div className="h-12 w-12 rounded-2xl bg-slate-950 flex items-center justify-center border border-slate-800 text-white font-black group-hover:border-indigo-500/50 transition-all text-xl">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <span className={`font-mono font-bold text-lg ${item.rank >= 3 ? "text-white" : "text-white"}`}>
                              {item.rank.toFixed(2)}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2 font-mono text-xs text-white bg-slate-950 px-4 py-1.5 w-fit rounded-xl border border-slate-800">
                        ...
                        <TableCell className="text-right pr-10">
                          <div className="flex justify-end gap-3">
                            <Sheet>
                              <SheetTrigger asChild>
                                <Button 
                                  ...
                                  className="bg-slate-950 border-slate-800 text-white hover:text-white transition-all rounded-xl px-5 h-11"
                                >
                            ...
                            <Button 
                              ...
                              className="w-11 h-11 text-white hover:text-white hover:bg-indigo-500/10 rounded-xl transition-all"
                            >
          ...
          <div className="flex items-center justify-between border-b border-slate-800 pb-8">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-white text-[10px] font-black uppercase tracking-widest">Domínios Favoritados p/ Auditoria Profunda</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {savedDomains.length === 0 ? (
              <div className="col-span-full py-24 text-center border-2 border-dashed border-slate-900 rounded-[2rem] opacity-30">
                ...
                <p className="text-white font-bold uppercase text-xs tracking-[0.3em]">Sua lista de oportunidades está limpa.</p>
              </div>
            ) : (
                ...
                <Card key={item.name} className="bg-slate-900 border-slate-800 p-8 rounded-[2rem] hover:border-amber-500/30 transition-all group overflow-hidden relative shadow-xl">
                  ...
                  <div className="flex justify-between items-start mb-8 relative z-10">
                     <div className="space-y-2">
                       ...
                       <div className="flex items-center gap-2">
                         <span className="text-[9px] font-black text-white uppercase tracking-widest px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg">High Value</span>
                         <span className="text-[9px] font-black text-white uppercase tracking-widest px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 rounded-lg">{item.rank} PR</span>
                       </div>
                     </div>
                     <button onClick={() => removeFromMonitor(item.name)} className="text-white hover:text-white transition-colors bg-slate-950 p-2 rounded-xl">
                  ...
                  <div className="flex gap-4 relative z-10">
                    <Button 
                      variant="outline" 
                      onClick={() => handleDeepAnalysis(item.name)}
                      className="flex-1 bg-slate-950 border-slate-800 text-white h-12 text-[10px] font-black uppercase tracking-widest hover:text-white rounded-xl"
                    >
                      Deep Scan
                    </Button>
                    <Sheet>
        ...
        <SheetHeader className="border-b border-slate-900 pb-12">
          <div className="flex items-center gap-8">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-white tracking-[.5em] uppercase mt-4">Auditoria Moz Expert</p>
            </div>
          </div>
        </SheetHeader>
        ...
        {isAnalyzing ? (
          <div className="flex flex-col items-center justify-center h-[50vh] gap-12">
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-white font-bold uppercase text-[10px] tracking-[0.5em]">Consultando Moz Scape V2 API (Padrão Ouro)</p>
            </div>
          </div>
        ) : analysisData ? (
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-16">
            ...
            <div className="grid sm:grid-cols-2 gap-8">
               <Card className="bg-slate-900 border-slate-800 p-12 flex flex-col items-center justify-center group overflow-hidden relative shadow-2xl rounded-[2.5rem]">
                  ...
                  <p className="text-[10px] font-black text-white uppercase tracking-[0.4em] mb-8">Score de Autoridade (DA)</p>
               ...
               <div className="grid grid-cols-1 gap-6">
                  <Card className="bg-slate-900 border-slate-800 p-8 flex flex-col justify-center space-y-2 rounded-[2rem] shadow-xl">
                     <span className="text-[10px] font-black text-white uppercase tracking-widest">Domínios de Referência</span>
                  ...
                  <Card className="bg-slate-900 border-slate-800 p-8 flex flex-col justify-center space-y-2 rounded-[2rem] shadow-xl">
                     <span className="text-[10px] font-black text-white uppercase tracking-widest">Métrica de Spam</span>
                     <span className={`text-5xl font-black tracking-tighter ${analysisData.backlinks.backlinks_spam_score > 10 ? 'text-white' : 'text-white'}`}>
            ...
            <div className="grid md:grid-cols-2 gap-12">
               <section className="space-y-6">
                  <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3 italic">
                    <History className="w-5 h-5 text-white" /> Histórico Host
                  </h3>
                  <div className="space-y-4">
                     <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex justify-between items-center">
                        <span className="text-xs font-bold text-white uppercase">Criação</span>
                     ...
                     <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex justify-between items-center">
                        <span className="text-xs font-bold text-white uppercase">Vencimento</span>
                        <span className="text-white font-bold">{analysisData.whois?.expiration_datetime ? new Date(analysisData.whois.expiration_datetime).toLocaleDateString() : 'N/A'}</span>
                     </div>
                  </div>
               </section>
               ...
               <section className="space-y-6">
                  <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3 italic">
                    <Globe className="w-5 h-5 text-white" /> Auditoria On-Page
                  </h3>
                  <Card className="bg-slate-900 border-none overflow-hidden rounded-2xl">
                    <div className="bg-slate-800 px-6 py-4 border-b border-indigo-500/10">
                       <h4 className="text-[10px] font-black text-white uppercase tracking-widest">Meta Title Tag</h4>
                    </div>
          ...
          <div className="flex flex-col items-center justify-center gap-8 h-[40vh]">
             <AlertCircle className="w-16 h-16 text-white opacity-50" />
             <div className="text-center space-y-2">
                <p className="text-xl font-black text-white uppercase tracking-tighter">Erro Crítico de Fetch</p>
                <p className="text-white text-[10px] font-bold uppercase tracking-widest max-w-[300px] leading-relaxed">Não foi possível carregar os dados. Verifique a chave de API ou se o domínio é válido.</p>
             </div>
             <Button variant="outline" onClick={() => handleDeepAnalysis(item.name)} className="border-slate-800 text-white hover:text-white rounded-xl px-10 h-14 uppercase font-black text-[10px] tracking-widest">Tentar Novamente</Button>
          </div>
        )}
      </div>
    </div>
         ...
         <SheetHeader className="border-b border-slate-900 pb-10">
            <div className="flex items-center gap-6">
               ...
               <div>
                  ...
                  <p className="text-white text-[10px] font-black uppercase tracking-widest mt-3">Notas Estratégicas e Planejamento</p>
               </div>
            </div>
         </SheetHeader>

         <div className="space-y-6">
            <label className="text-[10px] font-black text-white uppercase tracking-[0.2em]">Memorial Descritivo do Domínio</label>
            <textarea 
