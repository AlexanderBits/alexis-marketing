...
export default function ArquitetoBriefing() {
  ...
  // Detecta marcador de conclusão e exibe tela de revisão
  useEffect(() => {
      ...
      <header className="border-b border-white/10 px-4 py-4 flex items-center gap-3 bg-[#0A0A0A] sticky top-0 z-10">
        ...
        <div>
          ...
          <p className="text-white/50 text-xs">
            {showReview ? "Revise as informações do seu projeto" : "Consultoria para criação do seu site"}
          </p>
        </div>
      </header>
                    ...
                    {isUser ? (
                      ...
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          strong: ({ children }) => <strong className="text-[#D4FF33]">{children}</strong>,
                        }}
                      >
                        {msg.content}
                      </ReactMarkdown>
